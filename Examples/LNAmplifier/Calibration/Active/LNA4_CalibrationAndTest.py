...
History:
    ...
    03-07-2026  Added the board revision, manufacture date, and calibration date settings to the calibration program. Updated the program to use page-based EEPROM storage for improved efficiency when storing large datasets like frequency and gain arrays. Implemented a verification step to read back the stored EEPROM data and compare it against the original measurement data to ensure accurate storage. Added comprehensive error handling and reporting for all stages of the calibration process, including device connections, measurements, data storage, and verification. Updated user prompts and logging for clarity and guidance throughout the calibration workflow.
"""
...
def verify_eeprom_data(lna_device, port_index, expected_data, data_index, data_name):
    ...
    try:
        ...
        tolerance_percentage = 0.01  # 0.01% tolerance for verification
...
def main():
    ...
    try:
        ...
        bode_device.source_level = 9.0  # dBm
        ...
        # Warm-up measurement to prevent first real measurement timeout
        print("\n--- Warming up Bode100 measurement system ---")
        print("Performing dummy measurement to initialize instrument...", end=" ")
        try:
            # Save original settings
            original_point_count = bode_device.point_count
            original_start_frequency = bode_device.start_frequency
            
            # Configure for fast warm-up measurement
            bode_device.point_count = 51  # Use minimal points for speed
            bode_device.start_frequency = 100e3  # 100 kHz start frequency
            bode_device.write_properties()
            
            # Execute warm-up measurement
            warmup_result = bode_device.execute_sweep()
            
            if warmup_result is not None:
                print("✓")
                print("Bode100 measurement system ready")
            else:
                print("⚠")
                print("Warm-up measurement returned no data - first measurement may timeout")
            
        except Exception as e:
            print(f"⚠ Failed: {e}")
            print("First measurement may timeout - this is normal for initial connection")
            
        finally:
            # Always restore original settings
            try:
                bode_device.point_count = original_point_count
                if 'original_start_frequency' in locals():
                    bode_device.start_frequency = original_start_frequency
                bode_device.write_properties()
            except Exception as restore_error:
                print(f"⚠ Warning: Failed to restore Bode100 settings: {restore_error}")
                print("Measurement may proceed but settings should be verified")
        ...
        #Set the board revision if enabled
        if SET_BOARD_REVISION:
            print(f"\n--- Setting Board Revision to {BOARD_REVISION} ---")
            result = lna_device.set_board_revision(BOARD_REVISION, port_index)
            if result == 0:
                print(f"✓ Board revision set to {BOARD_REVISION} successfully")
            elif result is None:
                print("⚠ Failed to set board revision (exception occurred)")
            else:
                print(f"⚠ Failed to set board revision (error code: {result})")

        #Set manufacture date to today if enabled
        if SET_MANUFACTURE_DATE:
            print(f"\n--- Setting Manufacture Date to {MANUFACTURE_DATE} ---")
            result = lna_device.set_manufacture_date(MANUFACTURE_DATE, port_index)
            if result == 0:
                print(f"✓ Manufacture date set to {MANUFACTURE_DATE} successfully")
            elif result is None:
                print("⚠ Failed to set manufacture date (exception occurred)")
            else:
                print(f"⚠ Failed to set manufacture date (error code: {result})")

        # Set calibration date to today if enabled
        if SET_CALIBRATION_DATE:
            print(f"\n--- Setting Calibration Date to {CALIBRATION_DATE} ---")
            result = lna_device.set_calibration_date(CALIBRATION_DATE, port_index)
            if result == 0:
                print(f"✓ Calibration date set to {CALIBRATION_DATE}    successfully")
            elif result is None:
                print("⚠ Failed to set calibration date (exception occurred)")
            else:
                print(f"⚠ Failed to set calibration date (error code: {result})")
